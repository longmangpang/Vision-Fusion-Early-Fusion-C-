...
void YOLOPredictor::preprocessing(cv::Mat &image, float *&blob, std::vector<int64_t> &inputTensorShape)
{
    ...
    utils_my::letterbox(resizedImage, resizedImage, cv::Size((int)this->inputShapes[0][2], (int)this->inputShapes[0][3]),
                                                        ...
                                                        std::vector<Ort::Value> &outputTensors)
{
    ...
    for (int idx : indices)
    {
        ...
        utils_my::scaleCoords(res.box, res.boxMask, this->maskThreshold, resizedImageShape, originalImageShape);
        res.conf = confs[idx];
...
std::vector<Yolov8Result> YOLOPredictor::predict(cv::Mat &image)
{
    ...
    size_t inputTensorSize = utils_my::vectorProduct(inputTensorShape);
